...
namespace OW {
	...
	inline uintptr_t DecryptComponent(uintptr_t parent, uint8_t idx)
	{
		...
		auto dummy = SDK->RPM<_QWORD>(SDK->dwGameBase + 0x37714A0 + (Key1 >> 0x34));
		auto dummy2 = SDK->RPM<_QWORD>(SDK->dwGameBase + 0x37714A0 + (Key1 & 0xFFF));
		unsigned __int64 v10 = (unsigned int)v9 | v9 & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)((unsigned int)v9 + __ROL4__(dummy2, 1)) << 0x20);
		unsigned __int64  v12 = Key2 ^ ((unsigned int)v10 | v10 & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)(unsigned int)(2 * v10 + 0x690A393C) << 0x20));
		unsigned __int64  v13 = -(int)v7 & ((unsigned int)v12 | ((unsigned int)v12 | v12 & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)((unsigned int)v12 - (unsigned int)dummy) << 0x20)) & 0xFFFFFFFF00000000ui64 ^ ((unsigned __int64)((unsigned int)v12 ^ (unsigned int)~HIDWORD(dummy2)) << 0x20));
		return v13;
	}
	...
	inline unsigned __int64 __fastcall DecryptVis(unsigned __int64 a1)
	{
		...
		v3 = v2 + 0x8;
		v4 = SDK->RPM<_QWORD>(SDK->dwGameBase + 0x37724C0 + 8ull * (((uint8_t)a1 + 0x2F) & 0x7F) //a1 +- offset
		...
		return v4 ^ ~v3 ^ 0x248D9D283227B42F; //viskey 2
	}
	...
	inline uint64_t DecryptOutline(uint64_t a1)
	{
		...
		v3 = v2 + 0x8;
		v4 = 0i64;
		v5 = SDK->RPM<uintptr_t>(SDK->dwGameBase + 0x37724C0 + 8ull * (((uint8_t)a1 + 0x3A) & 0x7F) //a1 +- offset
		...
		return v5 ^ ~v3 ^ 0x8D9D2662D2D8EF3A; //outline key 2 
	}
